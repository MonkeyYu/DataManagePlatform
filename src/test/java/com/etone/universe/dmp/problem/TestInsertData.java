 ...
 */
public class TestInsertData implements Runnable {

	public TestInsertData(String filename, String tablename,
			String fileCloumnStr, int size) {
	...
	public void run() {
		...
		String querySql = "select linenum,rawdata from (select *,rank() OVER (PARTITION BY linenum ORDER BY cmdtime)n from err_sales_ext_t where filename like '%"
				+ filename
				+ "%' and errmsg='extra data after last expected column')t where n=1 order by linenum desc";
		...
		try {
			...
			// 查询数据
			List<Map<String, Object>> datas = QueryHelper.selectMap(conn,
					querySql);

			// 处理数据
			Map<String, String> sqlMap = insertData(conn, tablename,
					fileCloumnStr, size, datas);
	 ...
	 * @param datas 数据
	 */
	public static Map<String, String> insertData(Connection conn,
			String tableName, String fileCloumnStr, int size,
			List<Map<String, Object>> datas) {

		//先组织好insert语句前缀
		StringBuffer sBufferq = new StringBuffer();
		sBufferq.append("insert into ").append(tableName).append("(")
				.append(fileCloumnStr).append(") values(");
		...
		for (Map<String, Object> dataMap : datas) {
			...
			sBuffer.append("'")
					.append(sBufferEnd.substring(0, sBufferEnd.length() - 1))
					.append("')");
