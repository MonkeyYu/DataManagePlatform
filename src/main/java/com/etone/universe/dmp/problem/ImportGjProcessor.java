...
public class ImportGjProcessor {

	public static final Logger logger = LoggerFactory
			.getLogger(ImportGjProcessor.class);

	public ImportGjProcessor(Connection conn, Map<String, String> sqlMap,
			ProblemTask task) {
		this.conn = conn;
		this.sqlMap = sqlMap;
		this.task = task;
	 ...
	 * @throws Exception
	 */
	public void checkAndInsert(File file, List<Map<String, Object>> configList)
			throws Exception {
	 ...
	 * @return
	 */
	public String isDayFile(QueryCriteria criteria, String name,
			Map<String, String> reptables, String csvname) {
	 ...
	 * @throws Exception
	 */
	private void checkData(QueryCriteria criteria, String name,
			Map<String, Object> map, String vcshorttablename, String fullpath)
	 ...
	 * @throws SQLException
	 */
	private void createNullTbale(QueryCriteria criteria, String name,
			String vcshorttablename) throws SQLException {
	 ...
	 * @throws Exception
	 */
	private void loadData(QueryCriteria criteria, String name,
			Map<String, Object> map, String vcshorttablename, String fullpath,
			int flgInsert) throws SQLException, Exception {
		...
		processorFile(name, fullpath, iconv_fullpath, iconv_fullpath_1,
				tablename);
		...
		// 处理普通表
		String sc2 = processorTable(criteria, vcYear, vcshorttablename,
				tablename, randomStr);
		...
		// 处理外部表
		processorExtTable(criteria, name, tablename, isindoor, flgInsert,
				randomStr, sc2);
		...
		try {
			...
			dataLoadAfterThings(criteria, map, name, fullpath, startTime,
					tablename);
	 ...
	 * @param fullpath
	 */
	private void checkDirtribute(Map<String, Object> map, String name,
			String fullpath) {
		...
		if (Integer.parseInt(map.get("isdirtribute").toString()) == 0) {
			...
			for (int a = 0; a < arristripath.length; a++) {
				...
				if (Integer.parseInt(map.get("isinsert").toString()) == 1) {
					...
					excuteLiuxOrde3r("cp " + fullpath + " "
							+ map.get("databckpath").toString());
	...
	private String processorTable(QueryCriteria criteria, String vcYear,
			String vcshorttablename, String tablename, String randomStr)
			throws SQLException {

		String sc2 = "";
		// 获取表字段
		List<Map<String, Object>> fieldsList = task.selectMap(conn,
				sqlMap.get("getFields"), criteria.get("ytablename"));
		//普通建表语句
		...
		for (int l = 0; l < fieldsList.size(); l++) {
			...
			sqlCloumn.append(linkedHashMap.get("ecloumn").toString());
		...
		sqlCreateTab
				.append("with (appendonly=true,orientation=row,compresstype=zlib,compresslevel=5) ");
		sqlCreateTab
				.append("distributed randomly partition by list (intpid)(partition pp_197001 values(197001))");
		criteria.put("sql", sqlCreateTab);

		//判断是否存在目标表
		int flagExists = task.selectMap(conn, sqlMap.get("flagExists"),
				"%" + criteria.get("queryname")).size();
	 ...
	 * @param tablename
	 */
	private void processorFile(String name, String fullpath,
			String iconv_fullpath, String iconv_fullpath_1, String tablename) {
		...
		// 每行数据添加文件名
		String[] cmds3 = { "/bin/sh", "-c",
				"sed -i 's/^/" + name + "|&/g' " + task.getFilePath() + name };
		excuteLiuxOrde3r(cmds3);

		String[] cmdss = { "/bin/sh", "-c",
				"sed -i 's/\"//g' " + iconv_fullpath };
	...
	private void transferCharset(String fileName, String iconv_fullpath,
			String iconv_fullpath_1, String tablename) {

		// 需要特殊处理的表
		Map<String, String> transferTabls = ProblemUtil.getIdNames(
				task.getNamesFileName(), "transfercharsettables");

		if (transferTabls.get(tablename) == null) {
			// 普通文件：把文件转码为 utf-8
			String iconvStr = "iconv -c -f GBK -t utf-8 " + iconv_fullpath
					+ " -o " + " " + iconv_fullpath_1;
			excuteLiuxOrde3r(iconvStr);
			return;
		}

		// 告警文件：需要使用java的转码方式否则会出现换行
		...
		String[] cmds3 = { "/bin/sh", "-c",
				"sed -i 's/^/" + changeStr + "&/g' " + iconv_fullpath_1 };
		excuteLiuxOrde3r(cmds3);
	 ...
	 * @throws SQLException
	 */
	private void dataLoadAfterThings(QueryCriteria criteria,
			Map<String, Object> map, String name, String fullpath,
			long startTime, String tablename) throws SQLException {
		...
		//查询此次入库是否有异常
		List<Map<String, Object>> result = task.selectMap(conn,
				sqlMap.get("queryErrorList").toString(), "%" + name + "%");
		int errorList = 0;
		if (result != null) {
		...
		if (vctabletype.equals("告警")) {
			...
			//实行处理存储过程
			String execuSqlFunction = "select usp_gaojing_u_ltesource_day20170426('"
					+ yyyymmdd + "','" + tablename + "','" + name + "')";
			//处理告警数据
	 ...
	 * @throws SQLException
	 */
	private void insertData(QueryCriteria criteria, String tablename,
			int isindoor, String randomStr, String intpid) throws SQLException {
		String lsql = "";
		if (isindoor == 1) { //不需要关联ltesource资源表
			lsql = " insert into  " + tablename + " select t1.*, " + intpid
					+ " from " + randomStr + " t1";
			...
			lsql = " insert into  " + tablename + " select '室外',t1.*," + intpid
					+ " from " + randomStr + " t1";
		}
	...
	private void sensitivityFieldPro(String tablename) throws SQLException {
		...
		// 获取敏感信息表
		Map<String, String> sensitivityTables = ProblemUtil.getIdNames(
				task.getNamesFileName(), "sensitivitytablenames");
	 ...
	 */
	@SuppressWarnings({ "unchecked" })
	private void deleteData(QueryCriteria criteria, String tablename,
			String randomStr) throws SQLException {

		// 防止错删,将配置移至edmp用xml配置,只处理指定表
		Map<String, String> updateTables = ProblemUtil.getIdNames(
				task.getNamesFileName(), "tablenames");
		...
		if (updateTables.get(tablename) != null) {
			...
			// 调用建表方法
			processorTable(criteria, vcYear, criteria.get("vcshorttablename")
					.toString(), bakTablename, randomStr);
			...
			sbBuilder.append("delete from ").append(bakTablename)
					.append(" t where exists(select 1 from ").append(randomStr)
					.append(" t1 where 1=1 ");
			for (String field : fileds) {
				sbBuilder.append(" and t.").append(field).append("= t1.")
						.append(field);
			}
			sbBuilder.append(")");
	...
	public int checkLog(String name) throws SQLException {
		...
		//检查改文件是否已经入库过
		List<Map<String, Object>> result = task.selectMap(conn,
				sqlMap.get("checkFlgInsert"), "%" + name + "%");
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private void setParam(QueryCriteria criteria, String name, Map map,
			String vcshorttablename) {
	 ...
	 * @throws SQLException
	 */
	private void processorExtTable(QueryCriteria criteria, String name,
			String tablename, int isindoor, int flgInsert, String randomStr,
			String sc2) throws SQLException {
		...
		String createExtTableSql = sqlMap.get("createExtTable")
				.replace("${tableName}", randomStr)
				.replace("${columns}", olumns)
				.replace("${filePath}", extFilePath);

		criteria.put("sql", createExtTableSql);
		task.execute(conn, createExtTableSql);
		if (flgInsert > 0) {
			String del = "delete from " + tablename + " where filemaik='"
					+ name + "'";
		...
		List<Map<String, Object>> resultShu = task.selectMap(
				conn,
				sqlMap.get("getCountHangShu").toString() + " "
						+ criteria.get("randomStr"));

		int countHangShu = 0;
		if (resultShu != null) {
			countHangShu = Integer.valueOf(resultShu.get(0).get("num")
					.toString());
		}
	 ...
	 * @throws SQLException
	 */
	private void checkTablePar(QueryCriteria criteria, String tablename,
			String vcYear) throws SQLException {
		//检查这个是否有这一年的的分区表
		criteria.put("parTableName", "_1_prt_pp_" + vcYear);

		List<Map<String, Object>> resultCount = task.selectMap(conn,
				sqlMap.get("getGetPartTabCount"),
				"%" + criteria.get("queryname") + "%",
				"%" + criteria.get("parTableName") + "%");

		int exitCount = 0;
		if (resultCount != null) {
			exitCount = Integer.valueOf(resultCount.get(0).get("num")
					.toString());
		}
	 ...
	 * @throws SQLException
	 */
	private void createPartTable(QueryCriteria criteria, String tablename,
			String vcYear) throws SQLException {
		String months[] = { "01", "02", "03", "04", "05", "06", "07", "08",
				"09", "10", "11", "12" };
		for (int r = 0; r < months.length; r++) {
			String fenQuSql = " alter table " + tablename
					+ " add partition pp_" + vcYear + months[r] + " values ("
					+ vcYear + months[r] + ")";
	...
	private void excuteLogDataSql(QueryCriteria criteria) throws SQLException {
		task.execute(conn, sqlMap.get(ProblemUtil.LOGDATA_STR),
				criteria.get("vcfactory"), criteria.get("filename"),
				criteria.get("vcshorttablename"), criteria.get("vcctablename"),
				criteria.get("vcweidu"), criteria.get("vcetablename"),
				Integer.valueOf(criteria.get("intflagsprit").toString()),
				criteria.get("intfieldcount"), criteria.get("intflagsucc"),
				criteria.get("vcdescribe"), criteria.get("takkingtime"),
				criteria.get("databckpath"), criteria.get("countHangShu"),
