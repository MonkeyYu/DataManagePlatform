 ...
 */
public class SendWorkFileNowTask extends ProblemTask {
		...
		if (ProblemUtil.paramMap == null || ProblemUtil.paramMap.size() == 0) {
			try {
				...
				for (Map<String, Object> map : workList) {
					...
					/*// 删除全网指标问题点汇总表本工单记录
					execute(conn, sqlMap.get("deletecountqwzb"), orderCode);

					// 全网指标问题点汇总表
					String sql = sqlMap.get("countqwzb").replace("${tablename}",
							oederCodeMap.get("tableName").toString());
					execute(conn, sql, orderCode, orderCode);

					processorExtData(oederCodeMap.get("extFilePath").toString(),
							oederCodeMap.get("tableName").toString());*/
				}
			} catch (Exception e1) {
				e1.printStackTrace();
