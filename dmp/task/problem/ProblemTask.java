 ...
 */
public class ProblemTask extends BaseTask {

	public static final Logger logger = LoggerFactory
			.getLogger(ProblemTask.class);
	...
	*/
	...
	public void execute() {
		...
		// 设置日志信息
		event.setException(logInfoStr.toString());
		...
		event.setVclogid(logUUID);

		// 根据设置的流程状态去调用对应的job
		String jobName = judgeMap.get(judgeKey);
		if (!Mixeds.isNullOrEmpty(jobName)) {
			ProblemUtil.gotoJob(jobName);
		}

		Connection logConn = null;
		try {
			logConn = DB.getDataSource(logConnKey).getConnection();
			logConn.setAutoCommit(false);
			String logSql = "insert into d_problemDetailLog(vclogid,intnum,vcinfo,dtlogtime) values(?,?,?,?)";
			PreparedStatement prest = logConn.prepareStatement(logSql);
			for (DetailLog log : detailLogs) {
				prest.setString(1, logUUID);
				prest.setInt(2, log.getIntnum());
				prest.setString(3, log.getVcinfo());
				prest.setTimestamp(4, log.getLogtime());
				prest.addBatch();
			}
			prest.executeBatch();
			logConn.commit();
